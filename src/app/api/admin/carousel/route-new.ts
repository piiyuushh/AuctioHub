...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingImages = await CarouselImage.find({ isActive: true })
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (moveDirection) {
      ...
      const allImages = await CarouselImage.find({ isActive: true })
      const targetImage = allImages.find(img => img.order === targetOrder)
      if (!targetImage) {
        return NextResponse.json(
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const allImages = await CarouselImage.find({ isActive: true })
    const imagesToUpdate = allImages.filter(img => img.order > deletedOrder)
    for (const img of imagesToUpdate) {
      const imgId = img._id || img.id || ''
      if (imgId) {
        await CarouselImage.findByIdAndUpdate(imgId, { order: img.order - 1 })
      }
    }
